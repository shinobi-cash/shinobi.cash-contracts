...
/**
 * @title DepositInputSettler
 * @notice Handles cross-chain deposit intents on origin chain (where user has funds)
 * @dev User creates deposit intent, funds are escrowed, released to solver after pool deposit proof
 * @dev CRITICAL: intent.user = msg.sender ensures depositor address verification
 */
contract DepositInputSettler is IShinobiInputSettler {
    ...
    /// @notice Address of the ShinobiDepositEntrypoint (only caller allowed)
    ...
    /**
     * @notice Open a deposit intent and escrow funds
     * @dev CRITICAL: Can only be called by ShinobiDepositEntrypoint
     * @dev The entrypoint constructs the intent with verified depositor (msg.sender on entrypoint)
     * @param intent The deposit intent from the entrypoint
     */
    ...
    /**
     * @notice Finalize deposit intent after solver fills on destination (pool chain)
     * @dev Validates fill proofs (pool deposit proof) and releases escrowed ETH to solver
     * @param intent The original deposit intent
     * @param fillProofs Array of fill proofs from destination chain (pool)
     */
    function finalise(
    ...
    function refund(
        ...
        if (intent.refundCalldata.length == 0) {
            // Simple refund: transfer ETH back to depositor
